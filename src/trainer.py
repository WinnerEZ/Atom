...
class Trainer(object):
    ...
    def run(self):
        ...
        while True:
            ...
            # 由于加入噪音网络，不再需要epsilon
            if (network_type.startswith('dqn') and (self.params.recurrence != '')):
            ...
            for p in self.network.module.parameters():
                p.grad.data.clamp_(-5, 5)#将梯度限制在-5到5
    ...
    def evaluate_model(self, start_iter):
        ...
        if new_score > self.best_score:
            ...
            torch.save(self.network.module.state_dict(), model_path)#仅保存训练模型参数
        self.network.module.train()
        self.start_game()
